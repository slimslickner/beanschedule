...
def filter_occurrences_by_existing_transactions(
    ...
    Uses the same date-window logic as the hook's _match_ledger_transactions_lazy():
    a ledger transaction with matching schedule_id metadata is considered to cover
    an expected occurrence if its actual posting date is within date_window_days
    of the expected occurrence date. This handles cases where the actual payment
    date differs from the expected date (e.g., rent expected on the 1st, paid
    on the 3rd — still within a 3-day window).
    ...
    Args:
        schedule: The schedule to filter for (provides id and date_window_days)
    ...
    Returns:
        Filtered list of dates, with covered dates removed.
        Returns unmodified list if no actual transactions exist.
    """
    schedule_id = schedule.id
    date_window = schedule.match.date_window_days or 0

    # Collect actual (non-forecast) transactions with matching schedule_id
    schedule_txns = [
        entry
        for entry in entries
        if isinstance(entry, data.Transaction)
    ...
    # Filter out occurrences where an existing transaction falls within date_window_days
    result = []
    for occ_date in occurrence_dates:
        covered = any(
            abs((txn.date - occ_date).days) <= date_window for txn in schedule_txns
        )
        if not covered:

...
| `forecast_flag`           | string  | `#`     | Beancount flag set on all plugin-generated transactions (e.g. `*`, `P`)                                                                                                            |
...
3. **Creates transactions** - Generates forecast transactions (configurable flag, default `#`) with:
...
**Forecasts** (configurable flag, default `#`) are generated by the **plugin** during Beancount parsing. They show expected future transactions for budgeting and cash flow planning, starting from tomorrow. All forecast transactions carry the `#scheduled` tag.
